...
const cabangAktif = (daftarCabang || []).filter((c) => c.id <= 10 && c.nama_minutes);
...
function cariEmployee(providerMinutesId, providerName, cabangId) {
  ...
  const branchMatch = (daftarKaryawan || []).find(
    (k) => k.cabang_id === cabangId && normalisasi(k.nama_karyawan) === nama
  );
...
try {
  ...
  for (const cabang of cabangAktif) {
    try {
      ...
      if (!merchant) throw new Error(`Merchant tidak ditemukan: ${cabang.nama_minutes}`);
      ...
      const providers = await page.locator("table tbody tr").evaluateAll((rows) => {
        ...
        for (const row of rows) {
          ...
          if (!name) continue;
          found.push({ name, href: link.getAttribute("href") || "" });
        }
        return found;
      });
      ...
      for (const provider of providers) {
        ...
        const detail = await page.evaluate(() => {
          ...
          const table = tables.find((t) => {
            ...
            return heads.some((h) => h.includes("revenue share")) && heads.some((h) => h === "service" || h.includes("service"));
          });
          ...
          const iShare = idx("revenue share");
          const iSub = idx("sub income");
          const iTotal = idx("total income");
          ...
          for (const tr of table.querySelectorAll("tbody tr")) {
            ...
            if (iService >= 0) totalService += parse(cells[iService]);
            if (iShare >= 0) providerShare += parse(cells[iShare]);
            if (iSub >= 0) subIncome += parse(cells[iSub]);
            if (iTotal >= 0) totalIncome += parse(cells[iTotal]);
          }

          return { totalService, providerShare, subIncome, totalIncome };
        });

        if (!detail) {
          console.warn(`Detail tidak terbaca: ${provider.name}`);
          continue;
        }
      ...
      hasil.push({ cabang: cabang.nama_cabang, status: "OK", providers: saved });
      console.log(`✅ ${cabang.nama_cabang}: ${saved} provider`);
    } catch (error) {
...
const gagal = hasil.filter((x) => x.status === "ERROR");
